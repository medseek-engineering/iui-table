...
.directive('iuiPager', ['iuiTableLabels', function (iuiTableLabels) {
  ...
  return {
    ...
    link : function(scope, element, attrs) {
      ...
      scope.current.page = scope.page;
      scope.getFirstPage = function(){
        scope.current.page = scope.pageNum = 1;
      };
      scope.getPreviousPage = function(){
        scope.current.page = scope.pageNum = scope.current.page - 1;
      };
      scope.getNextPage = function(){
        scope.current.page = scope.pageNum = scope.current.page + 1;
      };
      scope.getLastPage = function(){
        scope.current.page = scope.pageNum = scope.pageCount();
      };
      scope.hasNoMore = function (){
        return (scope.current.page >= scope.pageCount());
      };
      scope.hasNoPrevious = function (){
        return (scope.current.page <= 1);
      };
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/$iui-table/templates/iui-pager.html',
    ...
    '		        <input type="number" id="tblpager_page_{{componentId}}" min="1" max="{{pageCount()}}" ng-model="current.page" ng-change="jumpToPage(current.page)" data-ng-disabled="hasNoPages()">\n' +
